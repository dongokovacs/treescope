...
test("sorting by height reorders cards, and direction toggle flips it", async ({ page }) => {
  ...
  await expect(page.locator("h3")).toHaveText(["Kocsányos tölgy", "Mezei juhar", "Valódi levendula"]);

  await page.getByRole("button", { name: "Csökkenő" }).click();
  await expect(page.locator("h3")).toHaveText(["Valódi levendula", "Mezei juhar", "Kocsányos tölgy"]);
});
